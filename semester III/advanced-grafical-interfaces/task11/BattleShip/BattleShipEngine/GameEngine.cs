...
namespace BattleshipEngine
{
    public class GameEngine
    {
        ...
        public void InitializePlayerAndComputerTileStates()
        {
        ...
        public List<Ship> CreateShips(List<Tuple<int, int>> selectedTiles)
        {
        ...
        public static List<Tuple<int, int>> GetShip(Tuple<int, int> startTile, List<Tuple<int, int>> selectedTiles)
        {
        ...
        public static List<Tuple<int, int>> GetNeighbors(Tuple<int, int> tile)
        {
        ...
        public static bool HasAdjacentShip(Tuple<int, int> tile, List<Tuple<int, int>> ship, List<Tuple<int, int>> selectedTiles)
        {
            var neighbors = GetNeighbors(tile);
        ...
        public (PlayerResponse, Tuple<int, int>) ComputerTurn()
        {
            ...
            if (hitShip)
            {
                ...
                {
                    return (PlayerResponse.Won, target);
                }
                if (hittedShip.IsAllTilesSunk())
                {
                    MarkSurroundingFieldsAsMissed(hittedShip);
                    return (PlayerResponse.HitSunk, target);
                }
                return (PlayerResponse.Hit, target);
            }
            else
            {
                playerTileState[target] = TileState.Miss;
                playerTurn = true;
                return (PlayerResponse.Miss, target);
            }
        }

        public void MarkSurroundingFieldsAsMissed(Ship ship)
        {
        ...
        public List<Tuple<int, int>> GetSurroundingTilesForShip(Tuple<int, int> shot)
        {
            Ship hitShip = GetHitShip(shot);
            ...
            var surroundingTiles = new List<Tuple<int, int>>();

            foreach (var tile in hitShip.Positions)
            {
                var neighbors = GetNeighbors(tile);

                foreach (var neighbor in neighbors)
                {
                    if (IsValidPosition(neighbor) && !surroundingTiles.Contains(neighbor))
                    {
        ...
        private Ship GetHitShip(Tuple<int, int> shot)
        {
            foreach (var ship in computerShips)
            {
                if (ship.Positions.Contains(shot))
